...
import { ArrowDown, ArrowUp, Beaker, CirclePlus, CornerDownRight, Play, Route, Rows3 } from 'lucide-react'
...
import type { InsertExposureSegment, SegmentType } from '@/types/segment'
...
export function PlansPage() {
  ...
  const [insertBefore, setInsertBefore] = useState<number>(appendPosition)
  ...
  const openSegmentForm = (before: number) => {
    if (!selected) return
    setInsertBefore(before)
    setSegmentForm({ ...segmentInitial, gas_mix: selected.breathing_mix })
    setLocalError(null)
    setFormMode(formMode === 'segment' ? null : 'segment')
  }
  ...
  const insertSegment = async (event: FormEvent) => {
    event.preventDefault(); if (!selected) return
    setBusy(true); setLocalError(null)
    const newSequenceNo = insertBefore === appendPosition ? segments.items.length + 1 : insertBefore
    const input: InsertExposureSegment = { ...segmentForm, plan_version: selected.version, before_sequence_no: insertBefore === appendPosition ? null : insertBefore }
    try {
      const updated = await segments.insert(selected.id, input)
      await plans.select(selected.id)
      setSegmentForm({ ...segmentInitial, gas_mix: selected.breathing_mix })
      setInsertBefore(appendPosition); setFormMode(null)
      const shifted = updated.length - newSequenceNo
      setNotice(shifted > 0
        ? `Segment inserted at sequence ${newSequenceNo}; ${shifted} later segment${shifted === 1 ? '' : 's'} shifted and input version advanced.`
        : `Segment appended at sequence ${newSequenceNo} and input version advanced.`)
    } catch (error) { setLocalError(error instanceof Error ? error.message : 'Segment insertion failed') }
    finally { setBusy(false) }
  }
      ...
      <div className="plan-workbench">
        ...
        <section className="sequence-board">
          {selected ? <>
            ...
            <div className="sequence-toolbar"><div><Rows3 size={17} /><span>{segments.items.length} ordered segments</span></div>{isPlanner && selected.plan_status === 'draft' && <div><Button size="small" startIcon={<CirclePlus size={16} />} onClick={() => openSegmentForm(appendPosition)}>Insert segment</Button><Button size="small" variant="contained" startIcon={<Play size={16} />} onClick={() => void model()} disabled={busy || segments.items.length === 0}>Run model</Button></div>}</div>
            {formMode === 'segment' && <form className="segment-form" onSubmit={insertSegment}>
              <span className="sequence-token">{String(insertBefore === appendPosition ? segments.items.length + 1 : insertBefore).padStart(2, '0')}</span>
              <TextField select size="small" label="Insert before" value={insertBefore} onChange={(event) => setInsertBefore(Number(event.target.value))}><MenuItem value={appendPosition}>Append at end</MenuItem>{segments.items.map((segment) => <MenuItem key={segment.id} value={segment.sequence_no}>Seq {segment.sequence_no} · {segment.segment_type}</MenuItem>)}</TextField>
              ...
              <Button type="submit" variant="contained" disabled={busy} startIcon={<CornerDownRight size={16} />}>{insertBefore === appendPosition ? 'Append' : `Insert before ${insertBefore}`}</Button>
            </form>}
            <div className="segment-table"><div className="segment-row segment-labels"><span>SEQ</span><span>TYPE / NOTE</span><span>DEPTH</span><span>TIME</span><span>ASCENT</span><span>GAS</span><span>ORDER</span></div>{segments.items.map((segment, index) => <div className="segment-row" key={segment.id}><span className="sequence-token">{String(segment.sequence_no).padStart(2, '0')}</span><div><strong>{segment.segment_type}</strong><small>{segment.notes || 'No note'}</small></div><span>{segment.depth_m.toFixed(1)} m</span><span>{segment.duration_min.toFixed(1)} min</span><span>{segment.ascent_rate_mmin.toFixed(1)}</span><span>O2 {(segment.gas_mix.o2 * 100).toFixed(0)} / He {(segment.gas_mix.he * 100).toFixed(0)}</span><div className="row-actions"><Tooltip title="Insert before this segment"><span><IconButton size="small" onClick={() => openSegmentForm(segment.sequence_no)} disabled={busy || selected.plan_status !== 'draft'}><CornerDownRight size={16} /></IconButton></span></Tooltip><Tooltip title="Move earlier"><span><IconButton size="small" onClick={() => void move(index, -1)} disabled={busy || index === 0 || selected.plan_status !== 'draft'}><ArrowUp size={16} /></IconButton></span></Tooltip><Tooltip title="Move later"><span><IconButton size="small" onClick={() => void move(index, 1)} disabled={busy || index === segments.items.length - 1 || selected.plan_status !== 'draft'}><ArrowDown size={16} /></IconButton></span></Tooltip></div></div>)}{segments.items.length === 0 && <div className="empty-state"><Beaker size={24} />No exposure segments recorded for this draft.</div>}</div>
